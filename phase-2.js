...
async function workout(totalTime) {
  try {
    let res1 = await stretch(totalTime);
    let res2 = await runOnTreadmill(res1);
    let res3 = await liftWeights(res2);
    console.log(`done working out with ${res3/1000} seconds left`);
  } catch (err) {
